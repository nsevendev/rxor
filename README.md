...
- Now `todo` and `todo2` are a reactive variable, of type ReaXarType and has methods  
...
myTodo.getTodo().value = [
  ...myTodo.getTodo().value,
  { id: 2, title: "todo 2", done: false } as TodoType,
...
class TodoService {
  ...
  toggleTodo = (id: number): void => {
    this.todos.value = this.todos.value.map((todo: TodoType) =>
    ...
    <>
      <form onSubmit={handlerSubmit}>
        <div>
          ...
          <button type={"button"} onClick={todoService.deleteTodoChecked}>
            Delete Todo Checked
          </button>
        </div>
      </form>
